...
import LoadMainMovie from './LoadMainMovie';
...
function MainMovie() {
    ...
    useEffect(() => {
        ...
        if (navigator.onLine) {
            ...
            sessionStorage.setItem('main_movie_index', String(index))
            setRandomIndex(index)
        } else {
            const savedIndex = sessionStorage.getItem('main_movie_index')
            setRandomIndex(Number(savedIndex))
        }
    }, [trendingMovie])
    ...
    if (isTrendingLoading || isLoading || randomIndex === null && !isTrendingError) return <LoadMainMovie />
    if (isTrendingError || isError || !data) return <SectionError onRetry={retryAll} />
